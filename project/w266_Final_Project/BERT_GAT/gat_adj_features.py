...
'''
TURN ON/OFF for Simple_GAT.py versus GAT_Opt.py
'''

class GATInputGenerator:
    def __init__(self):
        self.gat = "cora"
        self.FM = FeatureMatrix(base_path = "../Fake-News-Detection/dataset/")
        self.label_zip = None

    def getAdj(self, dataset="BuzzFeed"):
        if dataset == "BuzzFeed":
            adj_np = pd.read_csv("../../../../Fake-News-Detection/dataset/news_news_bf_adjacency_matrix.csv", header=None).values #GAT_Opt.py
#             adj_np = pd.read_csv("../Fake-News-Detection/dataset/news_news_bf_adjacency_matrix.csv", header=None).values #Simple_GAT.py
        else:
            adj_np = pd.read_csv("../Fake-News-Detection/dataset/news_news_pf_adjacency_matrix.csv", header=None).values

        return sp.csr_matrix(adj_np, dtype=int)

    def getFeatures(self, dataset="BuzzFeed"):
        feature_df, all_data_df = self.FM.get_feature_matrix(dataset)
        label = feature_df['label'].tolist()
        label_comp = label  #not sure why they are reversing it
#         label_comp = [0 if each else 1 for each in label]
        ...
        return sp.csr_matrix(feature_np, dtype=float).tolil(), label_comp, all_data_df
#         return sp.csr_matrix(feature_np, dtype=float).tolil()
    ...
    def getComps(self, dataset="BuzzFeed"):
        ...
        features, labels, all_data_df = self.getFeatures(dataset)
#         features = self.getFeatures(dataset)

        yTrain, yVal, yTest, train_mask, val_mask, test_mask = self.getYs(dataset)

#         print('************LABEL***********', labels)
        return adj, features, yTrain, yVal, yTest, train_mask, val_mask, test_mask, labels, all_data_df
        
#         print('*****LABEL ZIP *******', self.label_zip)
#         return adj, features, yTrain, yVal, yTest, train_mask, val_mask, test_mask, self.label_zip
